...
public static class AudioWaveformLoader
{
    ...
    public static float[]? ExtractFullWaveform(string wavFilePath, int resolution = 300)
    {
        if (string.IsNullOrWhiteSpace(wavFilePath)) return null;
        ...
        try
        {
            ...
            var samplesPerBin = (double)totalSamples / resolution;
            var result = new float[resolution];
            float maxRms = 0.001f;

            for (int bin = 0; bin < resolution; bin++)
            {
                int binStart = (int)(bin * samplesPerBin);
                int binEnd = Math.Min((int)((bin + 1) * samplesPerBin), totalSamples);
        ...
        {
            GD.PrintErr($"[WaveformLoader] Failed extracting full waveform: {ex.Message}");
            return null;
        }
    }
}
